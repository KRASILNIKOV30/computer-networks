...
- **DnsResolver** - главный класс для итеративного разрешения доменных имен
- **FileDesc** - RAII обёртка для системных вызовов (socket, close)
- **DnsResponse** - структуры для парсинга бинарных DNS ответов
- **Итеративный алгоритм** - рекурсивное разрешение от корневых серверов

### Системные вызовы и RAII:

- **socket(AF_INET, SOCK_DGRAM, 0)** - создание UDP сокета через FileDesc
- **sendto()** - отправка DNS запросов
- **recvfrom()** - получение DNS ответов  
- **setsockopt()** - установка таймаутов
- **close()** - автоматическое закрытие через RAII

### Особенности реализации:

- **UDP протокол** - DNS запросы по UDP на порт 53
- **Корневые DNS сервера** - встроенный список из 13 серверов
- **Рандомизация** - случайный порядок серверов для балансировки
- **DNS сжатие** - поддержка сжатых ссылок для оптимизации
- **Таймауты** - настраиваемое время ожидания (5 секунд)
- **Рекурсивный парсинг** - обработка DNS записей и сжатых ссылок
- **Глубина ограничения** - максимум 10 уровней для предотвращения зацикливания
